...
bool FeetInches::operator > (const FeetInches &dir){
    if(this->getFeet() > dir.getFeet()){
        return true;
    }else if( this->getFeet() == dir.getFeet()){
        if(this->getInches() > dir.getInches()){
            return true;
        }
    }
    return false;
...
bool FeetInches::operator == (const FeetInches &dir){
     if((this->getFeet() == dir.getFeet()) && (this->getInches() == dir.getInches())){
        return true;
    }
    return false;
};